@php
    ...
    $v5InitialSessionKeys = collect($v5InitialVariants)
        ...
        ->unique()
        ->values();
    $v5HasInitialSessionSelector = $v5InitialSessionKeys->count() > 1;
...
function syncSessionOptions(){const choices=sessionChoices(),field=$('#v5SessionField'),seen=new Set();if(field)field.hidden=choices.length<2;$$('[data-session-variant]').forEach(button=>{const candidate=JSON.parse(button.dataset.sessionVariant),key=packageKey(candidate),visible=choices.some(item=>packageKey(item)===key)&&!seen.has(key);button.hidden=!visible;if(visible)seen.add(key);button.classList.toggle('selected',key===sessionKey)})}
