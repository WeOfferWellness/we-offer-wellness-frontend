...
<script>
(()=>{const page=document.querySelector('#wowOfferingV5');if(!page)return;const cfg=JSON.parse(page.dataset.config),$=(s,r=page)=>r.querySelector(s),$$=(s,r=page)=>[...r.querySelectorAll(s)],money=n=>new Intl.NumberFormat('en-GB',{style:'currency',currency:cfg.currency||'GBP'}).format(Number(n||0)),holdKey=`wow-offering-hold:${cfg.source}:${cfg.id}`;let location=cfg.locations[0],variant=cfg.variants.find(x=>x.id===cfg.selectedVariantId)||cfg.variants[0],group=Math.max(3,Number((cfg.variants.find(x=>x.id===cfg.selectedVariantId)||cfg.variants[0]).group_min||3)),qty=1,availability={},date=null,time=null,reservation=null,expires=null,timer=null;try{const saved=JSON.parse(sessionStorage.getItem(holdKey)||'null');if(saved&&new Date(saved.expires)>new Date()){reservation=saved.reservation;expires=saved.expires;date=saved.date;time=saved.time;}}catch(e){}
...
renderCalendar=()=>{
    ...
    document.body.append(box);box.querySelector('.wow-v5__calendar-head h3').textContent=cfg.title||'Choose your session';
    const render=()=>{
