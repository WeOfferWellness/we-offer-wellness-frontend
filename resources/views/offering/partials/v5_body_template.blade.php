...
@media(max-width:700px){.wow-v5__booking-sheet{padding:0;align-items:flex-end}.wow-v5__booking-sheet-card{width:100%;max-height:94vh;border-radius:4px 4px 0 0}}
</style>
@endpush
@push('scripts')
<script>
...
renderCalendar=()=>{
    ...
    const render=()=>{
        ...
        slots.forEach(slot=>{const button=document.createElement('button');button.type='button';button.className=`wow-v5__calendar-slot${slot.start===time?' is-selected':''}`;button.textContent=slot.start;button.onclick=()=>{time=slot.start;sync();render()};times.append(button)});
    ...
    box.querySelector('[data-confirm]').onclick=async()=>{if(!date||!time)return;await hold();if(reservation){calendarView=null;box.remove();sync()}};
