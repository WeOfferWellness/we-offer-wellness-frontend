@php
    ...
    // Provider-entered catalogue copy occasionally includes decorative emoji.
    // The public V5 offering surface uses its own SVG/icon system, so keep
    // headings, the hero summary and rendered description clean text.
    $v5StripEmoji = static function (string $value): string {
        return trim((string) preg_replace('/[\p{Extended_Pictographic}\x{FE0F}\x{200D}]/u', '', $value));
    };
    $v5Title = $v5StripEmoji((string) ($offering['title'] ?? 'Offering'));
    ...
    $v5DescriptionSource = $v5StripEmoji(html_entity_decode(strip_tags((string) $v5DescriptionSource), ENT_QUOTES | ENT_HTML5, 'UTF-8'));
    ...
    $v5ExpectSource = $v5StripEmoji(html_entity_decode(strip_tags((string) $v5ExpectSource), ENT_QUOTES | ENT_HTML5, 'UTF-8'));
    $v5ExpectBlocks = [];
    $v5ExpectList = null;
    ...
    $v5IncludedSource = $v5StripEmoji(html_entity_decode(strip_tags((string) $v5IncludedSource), ENT_QUOTES | ENT_HTML5, 'UTF-8'));
    ...
    foreach ((array) ($offering['booking_variants'] ?? $offering['variants'] ?? []) as $i => $variant) {
        ...
        $configuredLabel = trim((string) ($variant['label'] ?? $variant['title'] ?? $variant['name'] ?? ''));
        $peopleLabel = match ($kind) { 'couple' => 'Couple', 'group' => 'Group', default => '1 Person' };
        // V3 price options combine delivery, people and (optionally) a
        // session package.  They are resolved as one price option at checkout,
        // but must be presented as separate customer choices.  Retain unusual
        // legacy labels only where no structured audience type exists.
        $label = $v5Source === 'v3' || $audienceType !== '' ? $peopleLabel : $configuredLabel;
        if ($label === '' || strtolower($label) === 'option') $label = $peopleLabel;
        $sessionPackageId = is_numeric($variant['session_package_id'] ?? null) ? (int) $variant['session_package_id'] : null;
        $sessionPackageKey = trim((string) ($variant['session_package_key'] ?? '')) ?: null;
        $sessionCount = is_numeric($variant['session_count'] ?? null) ? (int) $variant['session_count'] : null;
        $sessionLabel = trim((string) ($variant['session_package_name'] ?? ''));
        if ($sessionLabel === '' && ($sessionPackageId || $sessionPackageKey || $sessionCount)) {
            $sessionLabel = $configuredLabel;
        }
        if ($sessionLabel === '' && $sessionCount) {
            $sessionLabel = $sessionCount.' '.($sessionCount === 1 ? 'session' : 'sessions');
        }
        ...
        $sessionMetaParts = array_values(array_filter($sessionSelection, static fn (string $value): bool => strcasecmp(trim($value), $label) !== 0 && strcasecmp(trim($value), $sessionLabel) !== 0));
        ...
        $v5Variants[] = [
            ...
            'session_package_key' => $sessionPackageKey,
    ...
    $v5PeopleValue = static function (array $variants): ?string {
        ...
        // Use the customer-facing session names where a single mode is offered;
        // numerical ranges are reserved for genuinely continuous combinations.
        if (!$hasGroup) return $hasSolo && $hasCouple ? '1–2 people' : ($hasCouple ? 'Couple' : 'Solo');
    ...
    $v5InitialSessionKeys = collect($v5InitialVariants)
        ->map(fn (array $variant): string => (string) ($variant['session_package_key'] ?? ($variant['session_package_id'] ? 'id:'.$variant['session_package_id'] : '')))
        ->filter()
        ->unique()
        ->values();
    // A configured one-session package is still a deliberate booking choice.
    // Show the Session control for any package configuration, not only when
    // there is a second package to compare it with.
    $v5HasInitialSessionSelector = $v5InitialSessionKeys->isNotEmpty();
...
<style>
/* The buy box is a conversion control, not supporting copy: keep its V5
   geometry but make selections and booking details comfortably readable. */
.wow-v5__buybox .wow-v5__field>label{font-size:12px}.wow-v5__buybox .wow-v5__trigger{min-height:66px}.wow-v5__buybox .wow-v5__trigger strong{font-size:16px;line-height:1.15}.wow-v5__buybox .wow-v5__trigger small{font-size:12px;line-height:1.35}.wow-v5__buybox .wow-v5__option{min-height:58px}.wow-v5__buybox .wow-v5__option b{font-size:14px}.wow-v5__buybox .wow-v5__option small{font-size:11px}.wow-v5__buybox .wow-v5__summary{padding:13px}.wow-v5__buybox .wow-v5__summary dl{gap:9px 14px}.wow-v5__buybox .wow-v5__summary dt{font-size:12px}.wow-v5__buybox .wow-v5__summary dd{font-size:13px;font-weight:600}.wow-v5__buybox .wow-v5__qty{padding:12px}.wow-v5__buybox .wow-v5__qty b{font-size:14px!important}.wow-v5__buybox .wow-v5__qty small{font-size:11px!important}.wow-v5__buybox .wow-v5__qty-controls button,.wow-v5__buybox .wow-v5__qty-controls span{width:38px;height:38px;font-size:16px}.wow-v5__buybox .wow-v5__note{font-size:11px}
...
<main class="wow-v5" id="wowOfferingV5" data-config='@json($v5Config)'>
    ...
    <aside class="wow-v5__aside"><section class="wow-v5__buybox" id="v5Buybox"><div class="wow-v5__buyhead"><small>From</small><div class="wow-v5__price" id="v5Price">£{{ number_format($v5Price,2) }}</div><div class="wow-v5__status" id="v5Status">Checking availability</div></div><div class="wow-v5__buybody"><div class="wow-v5__field" id="v5LocationField"><label>Location</label><button class="wow-v5__trigger" id="v5LocationTrigger" type="button" aria-expanded="false"><span><strong id="v5LocationTitle">{{ $v5InitialLocation['label'] }}</strong><small id="v5LocationSub">{{ $v5InitialLocation['address'] }}</small></span><span>⌄</span></button><div class="wow-v5__menu" id="v5LocationMenu">@foreach($v5Locations as $location)<button class="wow-v5__option {{ $location['id'] === $v5InitialLocation['id'] ? 'selected' : '' }}" type="button" data-location-option="{{ $location['id'] }}"><span><b>{{ $location['label'] }}</b><small>{{ $location['address'] }}</small></span></button>@endforeach</div></div><div class="wow-v5__field"><label>People</label><button class="wow-v5__trigger" id="v5VariantTrigger" type="button" aria-expanded="false"><span><strong id="v5VariantTitle">{{ $v5InitialVariant['label'] }}</strong><small id="v5VariantMeta">{{ $v5InitialVariant['meta'] }}</small></span><span>⌄</span></button><div class="wow-v5__menu" id="v5VariantMenu">@foreach($v5Variants as $variant)<button class="wow-v5__option" type="button" data-variant='@json($variant)'><span><b>{{ $variant['label'] }}</b><small>{{ $variant['meta'] }}</small></span><b>£{{ number_format($variant['price'],2) }}{{ $variant['per_person']?' pp':'' }}</b></button>@endforeach</div></div><div class="wow-v5__field" id="v5SessionField" @unless($v5HasInitialSessionSelector) hidden @endunless><label>Session</label><button class="wow-v5__trigger" id="v5SessionTrigger" type="button" aria-expanded="false"><span><strong id="v5SessionTitle">{{ $v5InitialVariant['session_label'] ?? '' }}</strong><small id="v5SessionMeta">{{ !empty($v5InitialVariant['session_count']) ? $v5InitialVariant['session_count'].' '.($v5InitialVariant['session_count'] === 1 ? 'session' : 'sessions') : '' }}</small></span><span>⌄</span></button><div class="wow-v5__menu" id="v5SessionMenu">@foreach($v5Variants as $variant)@if(!empty($variant['session_label']))<button class="wow-v5__option" type="button" data-session-variant='@json($variant)'><span><b>{{ $variant['session_label'] }}</b><small>{{ !empty($variant['session_count']) ? $variant['session_count'].' '.($variant['session_count'] === 1 ? 'session' : 'sessions') : '' }}</small></span><b>£{{ number_format($variant['price'],2) }}{{ $variant['per_person']?' pp':'' }}</b></button>@endif @endforeach</div></div><div class="wow-v5__group" id="v5Group"><div style="display:flex;justify-content:space-between;margin-bottom:10px"><span><b>Group size</b><small style="display:block;color:#68736f;font-size:9px">Minimum 3{{ $v5GroupMax ? ' · maximum '.$v5GroupMax : '' }}</small></span><b id="v5GroupUnit"></b></div><div class="wow-v5__stepper"><button type="button" id="v5GroupMinus">−</button><input id="v5GroupInput" type="number" min="3" @if($v5GroupMax) max="{{ $v5GroupMax }}" @endif value="3" aria-label="Group size"><button type="button" id="v5GroupPlus">+</button></div></div><div class="wow-v5__summary"><dl><dt>Location</dt><dd id="v5SummaryLocation">{{ $v5InitialLocation['label'] }}</dd><dt>People</dt><dd id="v5SummaryVariant">{{ $v5InitialVariant['label'] }}</dd><template id="v5SummarySessionTemplate"><dt>Session</dt><dd id="v5SummarySession"></dd></template><dt>Date &amp; time</dt><dd id="v5SummaryDate">Choose after clicking Book now</dd><dt>Confirmation</dt><dd>Email confirmation</dd></dl></div><div class="wow-v5__qty"><span><b style="font-size:12px">Quantity</b><small style="display:block;color:#68736f;font-size:9px">Instant email confirmation</small></span><div class="wow-v5__qty-controls"><button type="button" id="v5QtyMinus">−</button><span id="v5Qty">1</span><button type="button" id="v5QtyPlus">+</button></div></div><button class="wow-v5__book" id="v5Book" type="button">Book now</button><p class="wow-v5__note" id="v5Note">Checking live availability…</p></div></section></aside></div>
...
<script>
(()=>{const page=document.querySelector('#wowOfferingV5');if(!page)return;const cfg=JSON.parse(page.dataset.config),$=(s,r=page)=>r.querySelector(s),$$=(s,r=page)=>[...r.querySelectorAll(s)],money=n=>new Intl.NumberFormat('en-GB',{style:'currency',currency:cfg.currency||'GBP'}).format(Number(n||0)),holdKey=`wow-offering-hold:${cfg.source}:${cfg.id}`;let location=cfg.locations.find(item=>item.id===cfg.selectedLocationId)||cfg.locations[0],variant=cfg.variants.find(x=>x.id===cfg.selectedVariantId)||cfg.variants[0],peopleKind=variant.kind,sessionKey=(variant.session_package_key|| (variant.session_package_id?`id:${variant.session_package_id}`:'')),group=Math.max(3,Number(variant.group_min||3)),qty=1,availability={},date=null,time=null,slotEnd=null,reservation=null,expires=null,timer=null;try{const saved=JSON.parse(sessionStorage.getItem(holdKey)||'null');if(saved&&new Date(saved.expires)>new Date()){reservation=saved.reservation;expires=saved.expires;date=saved.date;time=saved.time;slotEnd=saved.slot_end||null;}}catch(e){}
const setMenu=(trigger,menu)=>{if(!trigger||!menu)return;trigger.onclick=e=>{e.stopPropagation();const open=trigger.getAttribute('aria-expanded')==='true';$$('.wow-v5__menu').forEach(x=>x.classList.remove('open'));$$('.wow-v5__trigger').forEach(x=>x.setAttribute('aria-expanded','false'));menu.classList.toggle('open',!open);trigger.setAttribute('aria-expanded',String(!open))}};document.addEventListener('click',()=>{$$('.wow-v5__menu').forEach(x=>x.classList.remove('open'));$$('.wow-v5__trigger').forEach(x=>x.setAttribute('aria-expanded','false'))});setMenu($('#v5LocationTrigger'),$('#v5LocationMenu'));setMenu($('#v5VariantTrigger'),$('#v5VariantMenu'));setMenu($('#v5SessionTrigger'),$('#v5SessionMenu'));
...
function packageKey(item){return item.session_package_key||(item.session_package_id?`id:${item.session_package_id}`:'')}
function variantsForPeople(){const matches=variantsForLocation().filter(item=>item.kind===peopleKind);return matches.length?matches:variantsForLocation()}
function sessionChoices(){const seen=new Set();return variantsForPeople().filter(item=>{const key=packageKey(item);if(!key||seen.has(key))return false;seen.add(key);return true})}
function resolveVariant(){const candidates=variantsForPeople();const matching=sessionKey?candidates.find(item=>packageKey(item)===sessionKey):null;variant=matching||candidates[0];peopleKind=variant.kind;sessionKey=packageKey(variant);group=Math.max(3,Number(variant.group_min||3))}
...
function sync(){syncVariantUrl();const p=price(),sessions=sessionChoices(),sessionField=$('#v5SessionField'),sessionTitle=$('#v5SessionTitle'),sessionMeta=$('#v5SessionMeta');$('#v5Price').textContent=money(p);$('#v5MobilePrice').textContent=money(p);$('#v5LocationTitle').textContent=location.label;$('#v5LocationSub').textContent=location.address;$('#v5SummaryLocation').textContent=location.label;$('#v5VariantTitle').textContent=variant.label;$('#v5VariantMeta').textContent=variant.meta||'';$('#v5SummaryVariant').textContent=variant.kind==='group'?`${variant.label} · ${group} people`:variant.label;if(sessionField)sessionField.hidden=sessions.length===0;if(sessionTitle)sessionTitle.textContent=variant.session_label||'';if(sessionMeta)sessionMeta.textContent=variant.session_count?`${variant.session_count} ${variant.session_count===1?'session':'sessions'}`:'';let summarySession=$('#v5SummarySession');if(variant.session_label&&!summarySession){const template=$('#v5SummarySessionTemplate');if(template){template.after(template.content.cloneNode(true));summarySession=$('#v5SummarySession')}}const summarySessionLabel=$('#v5SummarySessionLabel');if(summarySession){summarySession.hidden=!variant.session_label;summarySession.textContent=variant.session_label||''}if(summarySessionLabel)summarySessionLabel.hidden=!variant.session_label;$('#v5Qty').textContent=qty;$('#v5Group').classList.toggle('show',variant.kind==='group');$('#v5GroupInput').value=group;$('#v5GroupUnit').textContent=variant.per_person?`${money(variant.price)} pp`:money(variant.price);$('#v5Status').textContent=live()?'Live availability':'Flexible booking';$('#v5Note').textContent=live()?'Book now opens the live calendar.':'The practitioner will arrange the date with you after booking.';$('#v5SummaryDate').textContent=date&&time?`${date} · ${time}`:'Choose after clicking Book now';syncVariantOptions();syncSessionOptions();updateQuickInfo()}
...
function renderMobileBooking(){
    queueMicrotask(()=>{const activeSheet=document.querySelector('.wow-v5__booking-sheet');const mobileBookButton=activeSheet?.querySelector('[data-book]');if(!activeSheet||!mobileBookButton)return;const mobileBookAction=document.createElement('div');mobileBookAction.className='wow-v5__booking-sheet-action';const mobileBookPrice=document.createElement('strong');mobileBookPrice.className='wow-v5__booking-sheet-price';mobileBookButton.parentNode.insertBefore(mobileBookAction,mobileBookButton);mobileBookAction.append(mobileBookPrice,mobileBookButton);const updateMobileBookPrice=()=>{mobileBookPrice.textContent=money(price())};updateMobileBookPrice();activeSheet.addEventListener('click',()=>window.setTimeout(updateMobileBookPrice,0));activeSheet.addEventListener('change',updateMobileBookPrice)});
...
function syncVariantOptions(){const available=variantsForLocation(),seen=new Set();$$('[data-variant]').forEach(button=>{const candidate=JSON.parse(button.dataset.variant),visible=available.some(item=>item.id===candidate.id)&&!seen.has(candidate.kind);button.hidden=!visible;if(visible)seen.add(candidate.kind);button.classList.toggle('selected',candidate.kind===peopleKind)})}
function syncSessionOptions(){const choices=sessionChoices(),field=$('#v5SessionField'),seen=new Set();if(field)field.hidden=choices.length===0;$$('[data-session-variant]').forEach(button=>{const candidate=JSON.parse(button.dataset.sessionVariant),key=packageKey(candidate),visible=choices.some(item=>packageKey(item)===key)&&!seen.has(key);button.hidden=!visible;if(visible)seen.add(key);button.classList.toggle('selected',key===sessionKey)})}
function selectLocation(next){location=next;const available=variantsForLocation();if(!available.some(item=>item.kind===peopleKind)){peopleKind=available[0].kind;sessionKey=''}resolveVariant();$$('[data-location-option]').forEach(button=>button.classList.toggle('selected',button.dataset.locationOption===location.id));$$('[data-location]').forEach(button=>button.classList.toggle('active',button.dataset.location===location.id));$('#v5MapTitle').textContent=location.online?'Join online':location.label;$('#v5MapCopy').textContent=location.online?'Your joining link is sent after booking.':location.address;date=time=slotEnd=null;syncMap();load();sync()}
function selectPeople(nextKind){peopleKind=nextKind;if(!variantsForPeople().some(item=>packageKey(item)===sessionKey))sessionKey='';resolveVariant();date=time=slotEnd=null;load();sync()}
function selectSession(nextKey){sessionKey=nextKey;resolveVariant();date=time=slotEnd=null;load();sync()}
$$('[data-location-option]').forEach(button=>button.onclick=()=>selectLocation(cfg.locations.find(item=>item.id===button.dataset.locationOption)||location));$$('[data-location]').forEach(button=>button.onclick=()=>selectLocation(cfg.locations.find(item=>item.id===button.dataset.location)||location));$$('[data-variant]').forEach(button=>button.onclick=()=>{if(!button.hidden)selectPeople(JSON.parse(button.dataset.variant).kind)});$$('[data-session-variant]').forEach(button=>button.onclick=()=>{if(!button.hidden)selectSession(packageKey(JSON.parse(button.dataset.sessionVariant)))});$('#v5GroupMinus').onclick=()=>{group=Math.max(3,group-1);sync();load()};$('#v5GroupPlus').onclick=()=>{group=Math.min(cfg.groupMax||Infinity,group+1);sync();load()};$('#v5GroupInput').onchange=e=>{group=Math.max(3,Math.min(cfg.groupMax||Infinity,Number(e.target.value)||3));sync();load()};$('#v5QtyMinus').onclick=()=>{qty=Math.max(1,qty-1);sync()};$('#v5QtyPlus').onclick=()=>{qty++;sync()};$('#v5Book').onclick=()=>reservation&&date&&time?add():(live()?renderCalendar():add());$('#v5MobileBook').onclick=()=>renderMobileBooking();syncVariantOptions();syncSessionOptions();$$('.wow-v5__faq-item button').forEach(b=>b.onclick=()=>{const i=b.parentElement;i.classList.toggle('open');$('b',b).textContent=i.classList.contains('open')?'−':'+'});
function enhanceMobileBookingSheet(){const sheet=document.querySelector('.wow-v5__booking-sheet');if(!sheet||sheet.dataset.peopleSessionReady)return;sheet.dataset.peopleSessionReady='1';const peopleOptions=sheet.querySelector('[data-variants]');const peopleLabel=peopleOptions?.previousElementSibling;if(peopleLabel)peopleLabel.textContent='People';const sessionWrap=document.createElement('div'),sessionLabel=document.createElement('label'),sessionOptions=document.createElement('div');sessionLabel.textContent='Session';sessionOptions.className='wow-v5__booking-sheet-options';sessionWrap.append(sessionLabel,sessionOptions);peopleOptions?.after(sessionWrap);const refresh=()=>{const available=variantsForLocation(),seenPeople=new Set();[...peopleOptions.querySelectorAll('button')].forEach((button,index)=>{const item=available[index];if(!item)return;button.hidden=seenPeople.has(item.kind);seenPeople.add(item.kind);button.classList.toggle('is-selected',item.kind===variant.kind)});peopleKind=variant.kind;sessionKey=packageKey(variant);const choices=sessionChoices();sessionWrap.hidden=choices.length===0;sessionOptions.replaceChildren();choices.forEach(item=>{const button=document.createElement('button');button.type='button';button.dataset.sessionKey=packageKey(item);button.textContent=`${item.session_label||`${item.session_count||1} session${item.session_count===1?'':'s'}`} · ${money(item.per_person&&item.kind==='group'?item.price*group:item.price)}`;button.classList.toggle('is-selected',button.dataset.sessionKey===sessionKey);sessionOptions.append(button)})};sessionOptions.addEventListener('click',event=>{const button=event.target.closest('[data-session-key]');if(!button)return;selectSession(button.dataset.sessionKey);refresh()});new MutationObserver(()=>queueMicrotask(refresh)).observe(peopleOptions,{childList:true});refresh()}
document.addEventListener('click',event=>{if(event.target.closest('#v5MobileBook'))queueMicrotask(enhanceMobileBookingSheet)});
